...
- `Product Assets`：飞书产品相关素材，包含 app icon、产品 logo、中文/英文/深色/白色变体。
- `General Icons`：功能、流程、状态、能力点图标，适合放在方案页、能力矩阵、流程图里。
- `Brand / Customer Logos`：客户/伙伴/外部品牌 logo，适合放在案例页、生态页、客户页里。
...
| `product-assets.json` | 产品素材分片索引，把同一产品的 app icon 和 logo 放在一起。 |
| `general-icons.json` | 通用功能/流程/状态图标分片索引。 |
| `brand-customer-logos.json` | 客户/伙伴/外部品牌 logo 分片索引。 |
...
- 原始图片：299 个
- 纳入公共池：215 个，其中 product assets 66 个、general icons 142 个、brand/customer logos 7 个
- 已有 TOS 链接：215 个
- 暂不纳入：84 个
...
2. 需要产品素材时读取 `product-assets.json`；需要通用图标时读取 `general-icons.json`；需要客户/品牌 logo 时读取 `brand-customer-logos.json`。
3. `icons.json` / `logos.json` 只作为兼容旧流程的物理类型分片。
4. 只有做全量审计、去重、重新分类时，才读取 `assets-index.json`。
5. CSV 只作为表格编辑/人工筛选用，不作为 LLM 默认上下文。

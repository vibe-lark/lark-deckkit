...
先读取 design.md 和 sdk/README.md。
这套项目的目标是：把飞书 PPT 的标准规范转成 HTML 幻灯片规范。

生成前先做一遍 front-design / 前端设计检查：
- 每页只讲一个观点
- 少放卡片，少放长段文字
- 字号按演示投屏来，不按普通网页来
- 页面要像 PPT，不要像后台 Dashboard

如果环境支持 skill，推荐同时参考：
https://skills.sh/anthropics/skills/frontend-design

优先使用 LarkSlideTemplates.createDeckFromOutline 生成第一版。
复杂页面再使用 visualLayout 和 components 精修。
所有文字和图形都要保持可编辑，不要做成整页截图。
...
不会用 Git 的话，也可以在 GitHub 页面点 `Code` -> `Download ZIP`，解压后运行上面的本地预览命令。
